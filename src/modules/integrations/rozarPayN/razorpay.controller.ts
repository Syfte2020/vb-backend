...
async verifys(@Param('subscription_id') id: string ,   
...
validate_coupon(@Body() body: any) {
  return this.razorpayService.validate_coupon(body);
}
...
@UseGuards(JwtAuthGuard)
    @Post('accept_terms_condition')
async accept_terms_condition(
  @Body() body: any,
   @CurrentUser() user: any,
) {
 return this.razorpayService.accept_terms_condition(
       body,
      user.id
    );
}
...
async verifyMandateOrder(
  ...
  return this.razorpayService.verifyMandateOrder(body);
}
...
async getMandate(
  ...
  return this.razorpayService.getMandate(id);
}
