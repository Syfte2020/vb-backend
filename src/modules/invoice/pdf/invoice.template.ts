...
/* -------------------------------------------------------
   Helpers — mirrors the frontend InvoicePage's own
   formatDate / roundAmount / money helpers exactly, so the
   PDF/email invoice always agrees with what the customer
   sees on the invoice page.
------------------------------------------------------- */
...
const roundAmount = (amount: any) => Math.round(Number(amount || 0));

const money = (amount: any) => roundAmount(amount);//formatPrice(roundAmount(amount));
...
export function invoiceTemplate(data: any) {
  ...
  const invoiceType =
    data?.booking_type === "booked" ? "Proforma Invoice" : "Reservation Invoice";

  const charges: any[] = data?.charges || [];

  // Non-addon charges
  const mainCharges: any[] = charges.filter(
    (c: any) => c.charge_type !== "addon"
  );

  // Addon charges
  const addons: any[] = charges.filter(
    (c: any) => c.charge_type === "addon"
  );
  ...
  // Total across ALL charges (main + addons) — matches the frontend's chargeTotal
  ...
  // Same fallback chain as the frontend (total_price -> price -> amount -> 0)
  const paxTotal = paxItems.reduce(
    (sum: number, item: any) =>
      sum + Number(item.total_price ?? item.price ?? item.amount ?? 0),
    0
  );
  ...
  /*
   * Prefer backend amount as the final invoice amount, falling back to the
   * computed sum. Same precedence as the frontend's grandTotal.
   */
  const grandTotal = roundAmount(
    data?.amount ?? chargeTotal + paxTotal + taxTotal
  );

  // Bill From — only render lines that actually have data, same as the frontend
  const billFromLines = [
    data?.venue_address || "",
    [data?.venue_city, data?.venue_state].filter(Boolean).join(", "),
    data?.venue_pincode || "",
    data?.email || "",
    data?.phone || "",
  ].filter(Boolean);
  ...
  const mainRows = mainCharges
    .map(
      ...
      <tr>
        ...
        <td class="p right">${money(charge.total_price)}</td>
      </tr>`
    )
    .join("");
  ...
  const addonSummaryRow =
    addonTotal > 0
      ...
      <tr>
        ...
        <td class="p right">${money(addonTotal)}</td>
      </tr>`
      : "";

  // Empty-state row, same condition as the frontend
  const noItemsRow =
    mainCharges.length === 0 && addonTotal === 0
      ? `
      <tr>
        <td class="p center" colspan="3">No invoice items</td>
      </tr>`
      : "";
...
${addons
            ...
            <tr>
              ...
              <td class="p center">${money(c.unit_price)}</td>
              <td class="p center">${c.quantity || 0}</td>
              <td class="p right">${money(c.total_price)}</td>
            </tr>`
            )
            .join("")}
        </tbody>
  ...
  // Pax snapshot table — now renders the actual price instead of "-"
  const paxTable =
    paxItems.length > 0
      ? `
...
${paxItems
            .map((item: any, i: number) => {
              const itemPrice =
                item.total_price ?? item.price ?? item.amount ?? 0;

              return `
            <tr>
              <td class="p">${i + 1}</td>
              <td class="p">${item.item_name || "-"}</td>
              <td class="p right">${money(itemPrice)}</td>
            </tr>`;
            })
            .join("")}
        </tbody>
...
${logoUrl ? `<img src="${logoUrl}" alt="Logo" />` : ""}
    ...
    <div class="header-right">
      ...
      <div>Invoice Date: <b>${formatDate(data?.orderDate)}</b></div>
    </div>
  </div>
  ...
  <div class="event-box">
    ...
    <b>Date:</b> ${formatDate(data?.fromDate)}<br/>
...
${noItemsRow}
  ...
  <div class="summary-wrap">
    <div class="summary">
      <div class="row"><span>Subtotal</span><span>${money(chargeTotal + paxTotal)}</span></div>
${taxRows}
      <hr />
      <div class="row grand">
