 ...
 /* Reel */
    (
        SELECT CONCAT(
            TRIM(TRAILING '/' FROM ?),
            '/',
            TRIM(LEADING '/' FROM vr.reel_url)
        )
        FROM venue_reels vr
        WHERE vr.child_venue_id = cv.child_venue_id
          AND vr.reel_url <> ''
        LIMIT 1
    ) AS videoUrl,
...
`;
  ...
  const likes = mapBounds ? await this.allLikedVenues(mapBounds) : [];
...
================================ */
  ...
  async userRecentViewsAPI(body: any, userId: number) {
    ...
    // Realtime update to the logged-in user
  ...
  async addLikedProperty(
    ...
    if (existing) {
      await this.dataSource.query(`DELETE FROM property_likes WHERE id = ?`, [
        existing.id,
      ]);
...
;
  ...
  async allLikedVenues(body: any) {
      ...
      INNER JOIN venue_child cv
    ON pl.property_id = cv.child_venue_id

INNER JOIN venue_parent pv
    ON pv.parent_venue_id = cv.parent_venue_id
      WHERE pv.lat BETWEEN ? AND ?
      AND pv.lng BETWEEN ? AND ?
    `,
      [body.south, body.north, body.west, body.east],
    );
